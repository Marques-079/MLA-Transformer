...
# torchrun --standalone --nproc_per_node=8 better_baseline.py
...
try:
    for step in range(start_step, max_steps):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            # Faster computation with minimal precision loss
            with torch.autocast(device_type=device_type, dtype=torch.bfloat16):
                logits, loss = model(x, y)
            loss = loss / grad_accum_steps
            loss_accum += loss.detach()

            # Active syncing on last micro-step only
        ...
        if master_process:
            ...
            log_line = (
                ...
                f"tokens/sec: {tokens_per_sec:.2f} | norm: {norm:.2f} | "
                f"time: {time_taken * 1000:.2f}ms"
            )
            print(log_line)

            # append to results.txt every 100 steps
            if step % 100 == 0:
                val_str = f"{last_val_loss:.6f}" if last_val_loss is not None else "nan"
                with open("results.txt", "a") as f:
                    # tab-separated for easy pandas import
                    f.write(
                        f"{step}\t"
    ...
    interrupted = True
    if master_process:
        print(f"\n[interrupt] Caught KeyboardInterrupt at step {step}, saving interrupt checkpoint...")
        # e.g. checkpoints/interrupt_step000123.pt
        save_ckpt(step, model, optimizer, extra_tag=f"interrupt_step{step:06d}")

finally:
    # Only save "final" checkpoint if we actually ran to completion
    if master_process and not interrupted and step >= 0:
